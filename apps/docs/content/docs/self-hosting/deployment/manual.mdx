...
description: Deploy signflow on a Linux server without Docker using Node.js and systemd.
---
...
Navigate to the `signflow` folder and create a `.env` file:
...
NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@localhost:5432/signflow"
NEXT_PRIVATE_DIRECT_DATABASE_URL="postgresql://user:password@localhost:5432/signflow"
NEXT_PRIVATE_SMTP_FROM_NAME="signflow"
NEXT_PRIVATE_SMTP_FROM_ADDRESS="noreply@your-domain.com"
```

<Callout type="info">
  If you use a reverse proxy in front of signflow, set `NEXT_PUBLIC_WEBAPP_URL` to the public URL
...
Create a service file to run signflow as a background service on Linux:

```ini
# /etc/systemd/system/signflow.service
...
Type=simple
User=www-data
WorkingDirectory=/var/www/signflow
EnvironmentFile=/var/www/signflow/.env
...
A minimal Nginx configuration for proxying to signflow:
