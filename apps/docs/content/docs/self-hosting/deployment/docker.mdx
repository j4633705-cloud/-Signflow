---
title: Docker
description: Run signflow as a standalone Docker container with an external database. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
The signflow image is available on both DockerHub and GitHub Container Registry:
...
docker pull ghcr.io/signflow/signflow:latest
```
...
These variables must be set for signflow to start:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL where signflow is accessible (e.g., `https://sign.example.com`) | -                             |
...
signflow requires two database connection strings:
...
| `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`      | Path to signing certificate inside container                   | `/opt/signflow/cert.p12` |
...
docker run -d \
  ...
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@db-host:5432/signflow" \
  ...
  -e NEXT_PRIVATE_SMTP_FROM_NAME="signflow" \
...
- **Volume mount** — mount a `.p12` file from the host into the container at `/opt/signflow/cert.p12` (shown above). This is the simplest approach for small to moderate deployments.
...
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@db-host:5432/signflow
...
signflow provides health check endpoints for monitoring:
...
```bash
docker inspect --format='{{.State.Health.Status}}' signflow
```

## Updating the Container

To update signflow to a new version:
...
The signflow container is stateless. All persistent data is stored in the PostgreSQL database.
...
<Accordions type="multiple">
  <Accordion title="Container fails to start">
    Check the container logs: `docker logs signflow`. Common causes: missing required environment
