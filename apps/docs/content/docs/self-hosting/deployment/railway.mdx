---
title: Railway
description: Deploy signflow on Railway with automatic builds from Git, managed PostgreSQL, and built-in networking. Use the one-click template or manual setup.
---
...
<Callout type="warn">
  ...
  [GitHub Discussions](https://github.com/j4633705-cloud/-Signflow/discussions) for help.
</Callout>
...
The fastest way to deploy signflow on Railway is using the official template:
...
Once deployed, Railway provides a URL like `signflow-production.up.railway.app`.

Click the URL in the Railway dashboard to access your signflow instance.
...
If you prefer more control over the setup, deploy signflow manually:
...
- Fork or select the [signflow repository](https://github.com/j4633705-cloud/-Signflow)
...
Click on the signflow service and go to the "Variables" tab. Add the following variables:
...
| `SIGNFLOW_DISABLE_TELEMETRY`     | Disable anonymous telemetry        | `false` |
...
signflow runs database migrations automatically on startup.
...
- Click on your signflow service
...
Railway provides several options for scaling your signflow deployment:
...
<Steps>
<Step>
### Open your signflow service

In the Railway dashboard, select the signflow service.
...
In the Railway dashboard, select your signflow service and open "Settings".
...
<Callout type="warn">
  signflow stores sessions in the database, so horizontal scaling works without additional
...
Railway uses usage-based pricing. Estimated monthly costs for signflow:
...
signflow requires a signing certificate to sign documents. On Railway, you can:

### Option 1: Use Generated Certificate (Default)

signflow generates a self-signed certificate automatically if none is provided. This works for testing but shows generic certificate information in signed documents.
...
In your signflow service variables, set:
...
<a href="https://app.koyeb.com/deploy?type=git&repository=github.com/j4633705-cloud/-Signflow&branch=main&name=signflow-app&builder=dockerfile&dockerfile=/docker/Dockerfile">
  <img
