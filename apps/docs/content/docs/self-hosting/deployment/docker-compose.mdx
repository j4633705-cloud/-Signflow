...
description: Deploy signflow with Docker Compose, including PostgreSQL. This production-ready setup is suitable for most self-hosted deployments.
---
...
curl -O https://raw.githubusercontent.com/signflow/signflow/release/docker/production/compose.yml
```

Alternatively, clone the full repository:

```bash
git clone https://github.com/j4633705-cloud/-Signflow.git
cd signflow/docker/production
```
...
services:
  ...
  signflow:
    ...
    ports:
      - ${PORT:-3000}:${PORT:-3000}
    volumes:
      - /opt/signflow/cert.p12:/opt/signflow/cert.p12:ro
...
| `signflow` | Main application container, waits for database to be healthy |

The signflow container waits for the database health check to pass before starting.
...
```bash
# Database (used by both database and signflow services)
...
# Public URL where signflow is accessible
NEXT_PUBLIC_WEBAPP_URL=https://sign.example.com
NEXT_PRIVATE_INTERNAL_WEBAPP_URL=http://localhost:3000

# Database connection (uses Docker service name)
NEXT_PRIVATE_DATABASE_URL=postgresql://signflow:your-secure-database-password@database:5432/signflow
...
sudo cp /path/to/your/cert.p12 /opt/signflow/cert.p12
sudo chown 1001:1001 /opt/signflow/cert.p12
sudo chmod 400 /opt/signflow/cert.p12
```
...
```
NAME                             STATUS              PORTS
signflow-production-database-1  running (healthy)   5432/tcp
signflow-production-signflow-1 running             0.0.0.0:3000->3000/tcp
```
...
```bash
docker compose --env-file .env restart signflow
```
...
For production, place signflow behind a reverse proxy for SSL termination:
...
```bash
# Manual backup
docker compose exec database pg_dump -U signflow signflow > backup.sql

# Restore from backup
docker compose exec -T database psql -U signflow signflow < backup.sql
```
...
```bash
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@your-db-host:5432/signflow
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@your-db-host:5432/signflow
```
...
<Accordions type="multiple">
  <Accordion title="Container fails to start">
    Check logs: `docker compose logs signflow`. Common causes: missing environment variables
  ...
  <Accordion title="Database connection errors">
    ...
    database psql -U signflow -d signflow -c "SELECT 1"`.
  </Accordion>
  ...
  <Accordion title="Emails not sending">
    ...
    password, check logs: `docker compose logs signflow | grep -i smtp`.
  </Accordion>
