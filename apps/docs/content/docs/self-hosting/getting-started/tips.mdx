...
description: Practical advice for running signflow in production.
---
...
<Callout type="error">
  This is the single most common issue when self-hosting signflow. Without a signing certificate,
  the application starts normally but document signing fails.
</Callout>

signflow does **not** bundle a [signing certificate](/docs/self-hosting/configuration/signing-certificate) in the Docker image. You must provide one yourself using one of two methods:
...
```bash
docker run -d \
  -v /path/to/your/cert.p12:/opt/signflow/cert.p12:ro \
  -e NEXT_PRIVATE_SIGNING_PASSPHRASE="your-certificate-password" \
  signflow/signflow:latest
```

The default certificate path inside the container is `/opt/signflow/cert.p12`. You can change this with `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`.
...
After starting signflow, check the certificate status endpoint:
...
Always set a password when creating your `.p12` certificate. When using OpenSSL, set the password during the export step and provide it to signflow via `NEXT_PRIVATE_SIGNING_PASSPHRASE`.

---

## Use the Health Check Endpoints

signflow exposes two endpoints for monitoring:
...
docker pull signflow/signflow:1.8.0

# Risky — may pull breaking changes
docker pull signflow/signflow:latest
```

Check the [GitHub releases](https://github.com/j4633705-cloud/-Signflow/releases) page for available versions.
...
Background jobs work by signflow sending HTTP requests to itself. If your reverse proxy or network setup causes issues with the app reaching its own public URL, set the internal URL:
...
NEXT_PRIVATE_UPLOAD_BUCKET=signflow-documents
...
signflow uses a PostgreSQL-based job queue by default (`local` provider). No Redis or external message broker is required for basic deployments.
...
```bash
docker run -it -e HOST=:: signflow/signflow:latest npm run start
```
...
The signflow container runs as a non-root user (UID 1001). If you mount files into the container (certificates, configuration), ensure they're readable:
