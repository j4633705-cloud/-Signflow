---
title: Quick Start
description: Get signflow running locally in minutes with Docker Compose.
---
...
docker compose -f docker/development/compose.yml exec database \
  psql -U signflow -d signflow -c "UPDATE \"User\" SET roles = '{ADMIN}' WHERE email = 'your@email.com';"
```
...
| ----------- | -------------------------------- | ----- |
| `signflow` | Main application                 | 3000  |
...
<Accordions type="multiple">
  ...
  <Accordion title="Container fails to start">
    Check the logs: `docker compose -f docker/development/compose.yml logs signflow`. Common
