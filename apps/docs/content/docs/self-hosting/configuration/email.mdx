...
signflow supports four email transport methods:
...
SMTP is the most common email transport. signflow supports two SMTP authentication methods.
...
Generate an API key in the Resend dashboard for signflow to use.
...
signflow uses React Email templates for all outgoing emails. The templates are located in `packages/email/templates/` and include:
...
- Custom logo replaces the signflow logo
...
Configure signflow to use the test server:
...
NEXT_PRIVATE_SMTP_FROM_NAME="signflow Test"
```
...
```bash
# Docker
docker logs signflow 2>&1 | grep -i email
...
For MailChannels, configure DKIM directly in signflow using the environment variables described above.
