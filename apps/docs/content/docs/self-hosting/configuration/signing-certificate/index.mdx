...
description: Configure the X.509 signing certificate used for digital signatures in self-hosted signflow.
---

import { Tab, Tabs } from 'fumadocs-ui/components/tabs';

When a document is completed in signflow, it is digitally signed using an X.509 certificate. This cryptographic signature provides:
...
Self-hosted signflow instances require a signing certificate. You can generate a self-signed certificate or purchase one from a Certificate Authority (CA).
...
For organisations requiring hardware-based key protection, signflow supports Google Cloud HSM. This provides:
