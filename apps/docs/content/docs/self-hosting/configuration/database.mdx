...
description: Configure PostgreSQL connection strings, pooling, SSL, migrations, and performance tuning for your signflow deployment.
---
...
signflow requires **PostgreSQL 14 or later**. No other databases are supported.
...
```
postgresql://signflow:password@localhost:5432/signflow
```

**Remote server with SSL:**

```
postgresql://signflow:password@db.example.com:5432/signflow?sslmode=require
```
...
```
postgresql://signflow:p%40ss%23word@localhost:5432/signflow
```

## Environment Variables

signflow uses two database connection variables:
...
signflow automatically detects common database environment variable formats used by hosting providers:
...
If your hosting provider sets these variables, signflow will use them automatically.
...
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@pooler-host:6432/signflow?pgbouncer=true

# Direct connection (bypasses PgBouncer)
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@db-host:5432/signflow
```
...
signflow uses Prisma, which maintains its own connection pool. Configure pool size with connection string parameters:

```
postgresql://user:password@host:5432/signflow?connection_limit=10&pool_timeout=30
```
...
```
postgresql://user:password@host:5432/signflow?sslmode=require
```
...
```
postgresql://user:password@host:5432/signflow?sslmode=verify-full&sslrootcert=/path/to/ca.crt
```
...
docker run -d \
  ...
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@host:5432/signflow?sslmode=verify-full&sslrootcert=/etc/ssl/certs/db-ca.crt" \
  signflow/signflow:latest
```

## Running Migrations

Database migrations update your schema when upgrading signflow.

### Automatic Migrations

When running signflow via Docker, migrations run automatically on container startup. No manual intervention is required.
...
<Accordions type="multiple">
  ...
  <Accordion title="Connection timeout during migration">
    ...
    ```bash
    NEXT_PRIVATE_DIRECT_DATABASE_URL="postgresql://user:password@host:5432/signflow?connect_timeout=60"
    ```
  </Accordion>
</Accordions>
...
<Accordions type="multiple">
  ...
  <Accordion title="Neon">
    ...
    **Configuration:**
    ...
    ```bash
    # Pooled connection
    NEXT_PRIVATE_DATABASE_URL=postgresql://[user]:[password]@[endpoint]-pooler.region.aws.neon.tech/signflow?sslmode=require

    # Direct connection for migrations
    NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://[user]:[password]@[endpoint].region.aws.neon.tech/signflow?sslmode=require
    ```
  ...
  <Accordion title="AWS RDS">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://signflow:[password]@your-instance.region.rds.amazonaws.com:5432/signflow?sslmode=require
    NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://signflow:[password]@your-instance.region.rds.amazonaws.com:5432/signflow?sslmode=require
    ```
  ...
  <Accordion title="Google Cloud SQL">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://signflow:[password]@/signflow?host=/cloudsql/[project]:[region]:[instance]
    ```
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://signflow:[password]@[public-ip]:5432/signflow?sslmode=require
    ```
  ...
  <Accordion title="Azure Database for PostgreSQL">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://signflow@[server-name]:[password]@[server-name].postgres.database.azure.com:5432/signflow?sslmode=require
    ```
  ...
  <Accordion title="DigitalOcean Managed Databases">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://doadmin:[password]@[cluster-host]:25060/signflow?sslmode=require
    ```
...
```bash
pg_dump -h host -U user -d signflow -F c -f documenso_backup.dump
```

**Restore from backup:**

```bash
pg_restore -h host -U user -d signflow -c documenso_backup.dump
```
...
Key parameters for signflow workloads:
...
signflow includes necessary indexes by default. Additional indexes may help for:
...
<Accordions type="multiple">
  ...
  <Accordion title="Authentication failed">
    ...
    Solutions:
    - Reset password with `ALTER USER signflow WITH PASSWORD 'newpassword';`
  ...
  <Accordion title="Permission denied">
    ...
    ```sql
    GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO signflow;
    GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO signflow;
    ```
  </Accordion>
</Accordions>
