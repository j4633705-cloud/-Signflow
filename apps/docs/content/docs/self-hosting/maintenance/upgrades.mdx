---
title: Upgrades
description: Upgrade your self-hosted signflow installation to get the latest features, security patches, and bug fixes.
---
...
signflow follows [Semantic Versioning](https://semver.org/):
...
<Callout type="info">
  Review the [release notes](https://github.com/j4633705-cloud/-Signflow/releases) before upgrading to
...
Check your running signflow version:

<Tabs items={['Docker', 'Docker Compose', 'Kubernetes']}>
<Tab value="Docker">

```bash
docker inspect signflow --format='{{.Config.Image}}'
```
...
```bash
kubectl get deployment signflow -n signflow -o jsonpath='{.spec.template.spec.containers[0].image}'
```
...
```bash
curl -s https://api.github.com/repos/signflow/signflow/releases/latest | grep tag_name
```

Or visit the [releases page](https://github.com/j4633705-cloud/-Signflow/releases).
...
```bash
curl -s https://hub.docker.com/v2/repositories/signflow/signflow/tags | jq -r '.results[].name'
```
...
```bash
curl -s https://api.github.com/orgs/signflow/packages/container/signflow/versions | jq -r '.[].metadata.container.tags[]'
```
...
```bash
docker compose exec database pg_dump -U signflow signflow > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
kubectl exec -n signflow deploy/postgres -- pg_dump -U signflow signflow > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
pg_dump "postgresql://user:password@host:5432/signflow" > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
kubectl get secret signflow-secrets -n signflow -o yaml > secrets-backup.yaml
kubectl get configmap signflow-config -n signflow -o yaml > configmap-backup.yaml
```
...
```bash
docker pull signflow/signflow:1.6.0
```
...
docker run -d \
  ...
  -p 3000:3000 \
  --env-file .env \
  -v /path/to/cert.p12:/opt/signflow/cert.p12:ro \
  signflow/signflow:1.6.0
```
...
services:
  signflow:
    image: signflow/signflow:${SIGNFLOW_VERSION:-latest}
```
...
```bash
docker compose ps
docker compose logs -f signflow
```
...
```bash
kubectl set image deployment/signflow \
  signflow=signflow/signflow:1.6.0 \
  -n signflow
```
...
```bash
kubectl rollout status deployment/signflow -n signflow
```

Watch pods transition:

```bash
kubectl get pods -n signflow -w
```
...
```bash
kubectl get pods -n signflow -o wide
```
...
signflow runs database migrations automatically when the container starts. No manual intervention is required.
...
```bash
docker logs signflow 2>&1 | grep -i migration
```
...
```bash
kubectl logs -l app.kubernetes.io/name=signflow -n signflow | grep -i migration
```
...
# Run migrations only
docker run --rm \
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@host:5432/signflow" \
  signflow/signflow:1.6.0 \
...
Read the [release notes](https://github.com/j4633705-cloud/-Signflow/releases) carefully.
...
curl -O https://raw.githubusercontent.com/signflow/signflow/main/.env.example
...
```bash
docker compose exec -T database psql -U signflow signflow < backup-20240115-120000.sql
```
...
```bash
psql "postgresql://user:password@host:5432/signflow" < backup-20240115-120000.sql
```
...
<Accordions type="multiple">
  <Accordion title="Container fails to start after upgrade">
    **Check logs:** `docker logs signflow`
  ...
  <Accordion title="Migration errors">
    ...
    **Test connectivity:**
    ...
    ```bash
    docker run --rm postgres:15 psql "postgresql://user:password@host:5432/signflow" -c "SELECT 1"
    ```
  ...
  <Accordion title="Application errors after upgrade">
    **If the application starts but behaves unexpectedly:**
    ...
    ```bash
    docker logs signflow 2>&1 | grep -i -E "(warn|error)"
    ```
  </Accordion>
