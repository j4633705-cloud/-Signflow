---
title: Backups
description: Backup strategies for signflow data.
---
...
signflow stores data in three locations:
...
```bash
pg_dump -h localhost -U signflow -d signflow -F c -f signflow_backup.dump
```
...
```bash
docker compose exec database pg_dump -U signflow -F c signflow > signflow_backup.dump
```
...
```bash
pg_dump -h localhost -U signflow -d signflow -F c \
  -f "signflow_$(date +%Y%m%d_%H%M%S).dump"
```
...
```bash
pg_dump -h localhost -U signflow -d signflow -F p -f signflow_backup.sql
```
...
```bash
pg_dump -h localhost -U signflow -d signflow | gzip > signflow_backup.sql.gz
```
...
```bash
cp /opt/signflow/cert.p12 /backup/signflow/cert.p12
```
...
```bash
echo "$NEXT_PRIVATE_SIGNING_LOCAL_FILE_CONTENTS" | base64 -d > /backup/signflow/cert.p12
```
...
```bash
aws s3 sync s3://your-signflow-bucket s3://your-backup-bucket
# or
aws s3 sync s3://your-signflow-bucket /backup/signflow/documents/
```
...
```bash
aws s3 sync s3://your-bucket /backup/signflow/documents/ \
...
Create a script to back up all signflow components:
...
S3_BUCKET="your-signflow-bucket"

# Create backup directory
mkdir -p "$BACKUP_DIR/$TIMESTAMP"

echo "Starting signflow backup: $TIMESTAMP"
...
if [ -f "/opt/signflow/cert.p12" ]; then
  echo "Backing up certificate..."
  cp /opt/signflow/cert.p12 "$BACKUP_DIR/$TIMESTAMP/cert.p12"
fi
...
tar -czf "$BACKUP_DIR/signflow_$TIMESTAMP.tar.gz" -C "$BACKUP_DIR" "$TIMESTAMP"
rm -rf "$BACKUP_DIR/$TIMESTAMP"

# Calculate checksum
sha256sum "$BACKUP_DIR/signflow_$TIMESTAMP.tar.gz" > "$BACKUP_DIR/signflow_$TIMESTAMP.sha256"

# Remove old backups
echo "Removing backups older than $RETENTION_DAYS days..."
find "$BACKUP_DIR" -name "signflow_*.tar.gz" -mtime +$RETENTION_DAYS -delete
find "$BACKUP_DIR" -name "signflow_*.sha256" -mtime +$RETENTION_DAYS -delete

echo "Backup completed: signflow_$TIMESTAMP.tar.gz"
```
...
```bash
#!/bin/bash
# signflow-backup-docker.sh

set -e

BACKUP_DIR="/backup/signflow"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
COMPOSE_PROJECT="signflow-production"
...
docker compose -p "$COMPOSE_PROJECT" exec -T database \
  pg_dump -U signflow -F c signflow > "$BACKUP_DIR/database_$TIMESTAMP.dump"

# Certificate backup (copy from container if needed)
docker compose -p "$COMPOSE_PROJECT" cp \
  signflow:/opt/signflow/cert.p12 "$BACKUP_DIR/cert_$TIMESTAMP.p12" 2>/dev/null || true
...
```bash
chmod +x /usr/local/bin/signflow-backup.sh
```
...
0 2 * * * /usr/local/bin/signflow-backup.sh >> /var/log/signflow-backup.log 2>&1

# Hourly backup (for high-activity instances)
0 * * * * /usr/local/bin/signflow-backup-hourly.sh >> /var/log/signflow-backup.log 2>&1
```

</Tab>
<Tab value="Systemd timer">

Create `/etc/systemd/system/signflow-backup.service`:

```ini
[Unit]
Description=signflow Backup

[Service]
Type=oneshot
ExecStart=/usr/local/bin/signflow-backup.sh
User=root
```

Create `/etc/systemd/system/signflow-backup.timer`:

```ini
[Unit]
Description=Daily signflow Backup
...
systemctl enable --now signflow-backup.timer
```
...
```bash
pg_restore -h localhost -U signflow -d signflow -c signflow_backup.dump
```

Docker Compose:

```bash
docker compose exec -T database pg_restore -U signflow -d signflow -c < signflow_backup.dump
```
...
```bash
psql -h localhost -U signflow -d signflow < signflow_backup.sql
```

For compressed backups:

```bash
gunzip -c signflow_backup.sql.gz | psql -h localhost -U signflow -d signflow
```
...
```bash
cp /backup/signflow/cert.p12 /opt/signflow/cert.p12
chmod 644 /opt/signflow/cert.p12
chown 1001:1001 /opt/signflow/cert.p12
```
...
```bash
aws s3 sync /backup/signflow/documents/ s3://your-signflow-bucket
```
...
```bash
docker compose exec -T database pg_restore -U signflow -d signflow -c < backup.dump
```
...
```bash
docker compose cp cert.p12 signflow:/opt/signflow/cert.p12
```
...
```bash
# Check checksum
sha256sum -c signflow_backup.sha256

# Verify pg_dump format
pg_restore --list signflow_backup.dump > /dev/null && echo "Backup is valid"
```
...
```bash
aws s3 cp signflow_backup.tar.gz s3://your-backup-bucket/signflow/
```
...
```bash
# Encrypt
gpg --symmetric --cipher-algo AES256 signflow_backup.tar.gz

# Decrypt
gpg --decrypt signflow_backup.tar.gz.gpg > signflow_backup.tar.gz
```
