...
```bash
pg_dump -h localhost -U signflow -d signflow -F c -f documenso_backup.dump
```
...
```bash
docker compose exec database pg_dump -U signflow -F c signflow > documenso_backup.dump
```
...
```bash
pg_dump -h localhost -U signflow -d signflow -F p -f documenso_backup.sql
```
...
```bash
pg_dump -h localhost -U signflow -d signflow | gzip > documenso_backup.sql.gz
```
...
```bash
pg_restore -h localhost -U signflow -d signflow -c documenso_backup.dump
```

Docker Compose:

```bash
docker compose exec -T database pg_restore -U signflow -d signflow -c < documenso_backup.dump
```
...
```bash
psql -h localhost -U signflow -d signflow < documenso_backup.sql
```

For compressed backups:

```bash
gunzip -c documenso_backup.sql.gz | psql -h localhost -U signflow -d signflow
```
...
```bash
aws s3 cp documenso_backup.tar.gz s3://your-backup-bucket/signflow/
```
