...
description: Diagnose and resolve common issues with your self-hosted signflow instance, organized by component.
---
...
```bash
kubectl logs -l app.kubernetes.io/name=signflow -n signflow -f
```
...
```bash
# Docker - find errors
docker logs signflow 2>&1 | grep -i error

# Docker - find warnings
docker logs signflow 2>&1 | grep -i warn

# Show last 100 lines
docker logs --tail 100 signflow
```

### Health Checks

signflow exposes a health endpoint to verify the application is running:
...
```bash
docker exec signflow printenv | grep NEXT
```
...
```bash
docker compose exec signflow printenv | grep NEXT
```
...
```bash
docker inspect signflow --format='{{.State.ExitCode}}'
```
...
```bash
# Docker
docker stats signflow

# Kubernetes
kubectl top pod -n signflow
```
...
<Steps>
  ...
  <Step>
    Change signflow's port mapping: `-p 3001:3000`
  </Step>
</Steps>
...
```
Error: password authentication failed for user "signflow"
```
...
```sql
ALTER USER signflow WITH PASSWORD 'newpassword';
```
...
```
Error: database "signflow" does not exist
```
...
```
Error: too many connections for role "signflow"
```
...
```
postgresql://user:password@host:5432/signflow?connection_limit=5
```
...
```bash
docker exec signflow printenv | grep SMTP
```
...
```bash
docker exec signflow nc -zv smtp.example.com 587
```
...
```bash
docker exec signflow printenv | grep SIGNING
```
...
```bash
docker exec signflow ls -la /opt/signflow/cert.p12
```
...
```
Error: EACCES: permission denied, open '/opt/signflow/cert.p12'
```
...
```
  ...
  <Step>
    Update the certificate file and restart signflow
  </Step>
</Steps>
...
```bash
docker exec signflow cat /etc/resolv.conf
```
...
- **[GitHub Discussions](https://github.com/j4633705-cloud/-Signflow/discussions)**
- **[Discord](https://signflow.com/discord)**: Join the signflow community Discord
- **[GitHub Issues](https://github.com/j4633705-cloud/-Signflow/issues)**: For confirmed bugs, open an issue
...
For security vulnerabilities, do not open a public issue. Contact security@signflow.com directly.
...
<Accordions type="multiple">
  <Accordion title="Check application logs for errors">
    Run `docker logs signflow` or `docker compose logs signflow` (or `kubectl logs` for
