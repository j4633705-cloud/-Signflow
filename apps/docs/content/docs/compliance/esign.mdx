...
## How signflow Supports Compliance

signflow provides features that support compliance with e-signature laws across jurisdictions:
...
signflow maintains an audit trail for each document, recording:
...
signflow supports compliance with Simple Electronic Signature (SES) requirements. The following are not currently provided:
