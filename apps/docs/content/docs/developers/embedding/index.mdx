...
signflow offers two types of embedding:
...
The [Platform Plan](https://signflow.com/platform-cta-pricing) adds enhanced customization:

- Custom CSS and styling variables
- Dark mode controls
- Removal of signflow branding
...
```jsx
import { EmbedDirectTemplate } from '@signflow/embed-react';
...
```jsx
import { EmbedSignDocument } from '@signflow/embed-react';
...
<Steps>
<Step>
Navigate to your team's templates in signflow
...
For example, `https://app.signflow.com/d/-WoSwWVT-fYOERS2MI37k` has the token `-WoSwWVT-fYOERS2MI37k`.
...
A [Web Components](https://developer.mozilla.org/en-US/docs/Web/API/Web_components) SDK (`@signflow/embed-webcomponent`) is also available for use outside of JavaScript frameworks. It works in any environment that supports custom elements.
...
| `host`                | `string`   | signflow instance URL. Defaults to `https://app.signflow.com`. |
...
| `language`            | `string`   | Set the UI language. See [Supported Languages](https://github.com/j4633705-cloud/-Signflow/tree/main/packages/lib/constants/locales.ts). |
