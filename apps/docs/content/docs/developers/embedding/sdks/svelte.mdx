---
title: Svelte
description: Embed signflow signing in your Svelte application using the official SDK.
---
...
<Tabs items={['npm', 'yarn', 'pnpm']}>
  <Tab value="npm">``` npm install @signflow/embed-svelte ```</Tab>
  <Tab value="yarn">``` yarn add @signflow/embed-svelte ```</Tab>
  <Tab value="pnpm">``` pnpm add @signflow/embed-svelte ```</Tab>
</Tabs>
...
```svelte
<script lang="ts">
  import { EmbedDirectTemplate } from '@signflow/embed-svelte';
...
```svelte
<script lang="ts">
  import { EmbedSignDocument } from '@signflow/embed-svelte';
