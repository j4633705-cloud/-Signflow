...
Verifying webhook requests ensures that incoming payloads originate from signflow and have not been tampered with. Without verification, attackers could forge requests to your endpoint and trigger unintended actions.

## How signflow Signs Webhooks

When you configure a webhook with a secret, signflow includes that secret in every webhook request via the `X-signflow-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.

```bash
POST /your-webhook-endpoint HTTP/1.1
Host: your-server.com
Content-Type: application/json
X-signflow-Secret: your_webhook_secret_here
...
| Header               | Description                                             |
| -------------------- | ------------------------------------------------------- |
| `X-signflow-Secret` | The secret key you configured when creating the webhook |
...
<Steps>
  <Step>
    Extract the `X-signflow-Secret` header from the incoming request
  </Step>
  <Step>
...
const WEBHOOK_SECRET = process.env.SIGNFLOW_WEBHOOK_SECRET;
...
app.post('/webhooks/signflow', (req, res) => {
  const receivedSecret = req.headers['x-signflow-secret'];
...
WEBHOOK_SECRET = os.environ.get('SIGNFLOW_WEBHOOK_SECRET')
...
def handle_webhook():
    received_secret = request.headers.get('X-signflow-Secret', '')
...
| --------------- | --------------------------------------- | ------------------------------------------------------------------ |
| Secret mismatch | Webhook secret changed or misconfigured | Verify the secret in your environment matches the one in signflow |
| Empty header    | Webhook created without a secret        | Add a secret to the webhook configuration in signflow             |
...
3. Update the webhook configuration in signflow
...
app.post('/webhooks/signflow', webhookLimiter, (req, res) => {
  // Handle webhook...
});
```
