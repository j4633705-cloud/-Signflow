...
1. You configure a webhook URL in signflow
2. When an event occurs, signflow sends an HTTP POST to your URL
3. Your application processes the event and responds with 200 OK

signflow supports webhook events for the full document lifecycle (created, sent, opened, signed, completed, rejected, cancelled) as well as template events (created, updated, deleted, used).
