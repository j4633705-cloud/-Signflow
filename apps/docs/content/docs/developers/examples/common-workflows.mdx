...
app.post('/webhooks/signflow', (req, res) => {
  ...
  const secret = req.headers['x-signflow-secret'] as string;
...
Register your endpoint in the signflow dashboard:
...
<Steps>
  ...
  <Step>
    Enter your webhook URL (e.g. <code>https://your-app.com/webhooks/signflow</code>)
  </Step>
  <Step>
  ...
  <Step>
    Enter your webhook secret (the same value stored in <code>SIGNFLOW_WEBHOOK_SECRET</code>)
  </Step>
  <Step>
    Click **Create Webhook**
  </Step>
...
class SignFlowApiError extends Error {
  code?: string;
  statusCode: number;
  details?: Record<string, unknown>;
...
try {
  ...
  if (error instanceof SignFlowApiError) {
...
- [API Reference](https://openapi.signflow.com/) - Full endpoint documentation
