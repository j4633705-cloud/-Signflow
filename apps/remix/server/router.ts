import { tsRestHonoApp } from '@signflow/api/hono';
import { auth } from '@signflow/auth/server';
import { jobsClient } from '@signflow/lib/jobs/client';
import { LicenseClient } from '@signflow/lib/server-only/license/license-client';
import { createRateLimitMiddleware } from '@signflow/lib/server-only/rate-limit/rate-limit-middleware';
import {
  aiRateLimit,
  apiTrpcRateLimit,
  apiV1RateLimit,
...
import { TelemetryClient } from '@signflow/lib/server-only/telemetry/telemetry-client';
import { migrateDeletedAccountServiceAccount } from '@signflow/lib/server-only/user/service-accounts/deleted-account';
import { migrateLegacyServiceAccount } from '@signflow/lib/server-only/user/service-accounts/legacy-service-account';
import { env } from '@signflow/lib/utils/env';
import { logger } from '@signflow/lib/utils/logger';
import { openApiDocument } from '@signflow/trpc/server/open-api';
...
// Can be disabled by setting SIGNFLOW_DISABLE_TELEMETRY=true
