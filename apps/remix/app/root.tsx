...
import { SessionProvider } from '@signflow/lib/client-only/providers/session';
import { APP_I18N_OPTIONS, type SupportedLanguageCodes } from '@signflow/lib/constants/i18n';
import { createPublicEnv } from '@signflow/lib/utils/env';
import { extractLocaleData } from '@signflow/lib/utils/i18n';
import { TrpcProvider } from '@signflow/trpc/react';
import { getOrganisationSession } from '@signflow/trpc/server/organisation-router/get-organisation-session';
import { Toaster } from '@signflow/ui/primitives/toaster';
import { TooltipProvider } from '@signflow/ui/primitives/tooltip';
...
export function LayoutContent({ children }: { children: React.ReactNode }) {
  ...
  // Recipient routes (signing pages) put `signflow-branded` on <body> so the
  ...
  return (
    <html translate="no" lang={lang} data-theme={theme} className={theme ?? ''}>
      ...
      <body className={isRecipientRoute ? 'signflow-branded' : undefined}>
