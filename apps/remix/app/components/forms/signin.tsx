...
import { AuthenticationErrorCode } from '@signflow/auth/server/lib/errors/error-codes';
...
import { ZCurrentPasswordSchema } from '@signflow/trpc/server/auth-router/schema';
...
import { PinInput, PinInputGroup, PinInputSlot } from '@signflow/ui/primitives/pin-input';
    ...
    <Form {...form}>
      <form className={cn('flex w-full flex-col gap-y-4', className)} onSubmit={form.handleSubmit(onFormSubmit)}>
        <fieldset className="flex w-full flex-col gap-y-4" disabled={isSubmitting || isPasskeyLoading}>
          ...
          <Button type="submit" size="lg" loading={isSubmitting} className="dark:bg-signflow dark:hover:opacity-90">
