...
| Signing pages (no custom logo) | signflow fallback shown | Fallback keeps its internal `/` link |
...
- On signing surfaces, render a configured custom branding logo as a plain image with no
  link wrapper. Leave the signflow fallback logo's internal `/` link untouched.
...
- New shared component `packages/email/template-components/template-branding-logo.tsx`
  ...
  - the signflow fallback logo (`/static/logo.png`) when custom branding is disabled or
    no logo is set.
...
- `apps/remix/app/components/general/envelope-signing/envelope-signer-header.tsx` (V2,
  ...
  signflow fallback keeps its `<Link to="/">`.
...
- V2 with no custom logo: signflow fallback still links to `/`.
...
- signflow fallback logos continue linking to `/`.
