...
export const reregisterEmailDomain = async ({ emailDomainId }: ReregisterEmailDomainOptions) => {
  const encryptionKey = SIGNFLOW_ENCRYPTION_KEY;

  if (!encryptionKey) {
    throw new Error('Missing SIGNFLOW_ENCRYPTION_KEY');
  }
  ...
  // The selector field in the DB is the full record name (e.g. "signflow-orgid._domainkey.example.com").
