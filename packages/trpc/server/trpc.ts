...
import { getApiTokenByToken } from '@signflow/lib/server-only/public-api/get-api-token-by-token';
import { assertUserNotDisabled } from '@signflow/lib/server-only/user/assert-user-not-disabled';
import type { TrpcApiLog } from '@signflow/lib/types/api-logs';
import type { ApiRequestMetadata } from '@signflow/lib/universal/extract-request-metadata';
import { alphaid } from '@signflow/lib/universal/id';
...
export const authenticatedMiddleware = t.middleware(async ({ ctx, next, path, meta }) => {
  ...
  // Taken from `authenticatedMiddleware` in `@signflow/api/v1/middleware/authenticated.ts`.
