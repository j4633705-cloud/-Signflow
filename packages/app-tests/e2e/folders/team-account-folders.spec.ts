...
import { DocumentVisibility, FolderType, TeamMemberRole } from '@signflow/prisma/client';
import { seedBlankDocument, seedTeamDocuments } from '@signflow/prisma/seed/documents';
...
test('[TEAMS]: can create a document inside a document folder', async ({ page }) => {
  ...
  await fileChooser.setFiles(path.join(__dirname, '../../../assets/signflow-supporter-pledge.pdf'));

  await page.waitForTimeout(3000);

  await expectTextToBeVisible(page, 'signflow-supporter-pledge.pdf');
...
test('[TEAMS]: can pin a document folder', async ({ page }) => {
  ...
  await expect(page.locator('svg.text-signflow.h-3.w-3')).toBeVisible();
});
...
test('[TEAMS]: can unpin a document folder', async ({ page }) => {
  ...
  await expect(page.locator('svg.text-signflow.h-3.w-3')).not.toBeVisible();
});
