...
import { seedPendingDocumentWithFullFields } from '@signflow/prisma/seed/documents';
import { seedDirectTemplate } from '@signflow/prisma/seed/templates';
import { seedUser } from '@signflow/prisma/seed/users';

const BRANDING_URL = 'https://brand.example/signing?source=signflow';
...
test('[SIGNING_BRANDING]: V1 normal signing renders custom logo as a plain image', async ({ page }) => {
  ...
  const { recipients } = await seedPendingDocumentWithFullFields({
    owner: user,
    teamId: team.id,
    recipients: ['v1-branding-signer@test.signflow.com'],
...
test('[SIGNING_BRANDING]: V2 signing renders custom logo as a plain image', async ({ page }) => {
  ...
  const { recipients } = await seedPendingDocumentWithFullFields({
    owner: user,
    teamId: team.id,
    recipients: ['v2-branding-signer@test.signflow.com'],
    fields: [FieldType.SIGNATURE],
...
test('[SIGNING_BRANDING]: V2 signing keeps internal link for the signflow fallback logo', async ({ page }) => {
  const { user, team } = await seedUser();

  const { recipients } = await seedPendingDocumentWithFullFields({
    owner: user,
    teamId: team.id,
    recipients: ['v2-fallback-signer@test.signflow.com'],
    fields: [FieldType.SIGNATURE],
    updateDocumentOptions: { internalVersion: 2 },
  });
...
test('[SIGNING_BRANDING]: embedded signing does not render custom logo Brand Website links', async ({ page }) => {
  ...
  const { recipients } = await seedPendingDocumentWithFullFields({
    owner: user,
    teamId: team.id,
    recipients: ['embed-branding-signer@test.signflow.com'],
    fields: [FieldType.SIGNATURE],
    updateDocumentOptions: { internalVersion: 2 },
  });
