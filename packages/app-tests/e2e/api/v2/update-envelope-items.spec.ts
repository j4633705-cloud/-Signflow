...
import { DocumentStatus, EnvelopeType, FieldType, RecipientRole } from '@signflow/prisma/client';
...
import type { TUpdateEnvelopeItemsRequest } from '@signflow/trpc/server/envelope-router/update-envelope-items.types';
...
const distributeEnvelope = async (request: APIRequestContext, authToken: string, envelopeId: string) => {
  const recipientEmail = `signer-${Date.now()}@test.signflow.com`;
