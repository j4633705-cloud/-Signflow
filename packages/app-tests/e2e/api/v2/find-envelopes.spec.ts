...
import { DocumentStatus, DocumentVisibility, EnvelopeType, TeamMemberRole } from '@signflow/prisma/client';
import {
  seedBlankDocument,
...
import { seedTeam, seedTeamEmail, seedTeamMember } from '@signflow/prisma/seed/teams';
...
test.describe('Find Envelopes API - Team Email', () => {
  test('should include envelopes received by team email from external senders', async ({ request }) => {
    const { team, owner } = await seedTeam();
    const teamEmailAddr = `team-find-env-${team.id}@test.signflow.com`;
  ...
  test('should NOT include external noise from other teams when team has team email', async ({ request }) => {
    ...
    const teamEmailAddr = `team-noise-${teamA.id}@test.signflow.com`;
  ...
  test('team email received docs bypass visibility for managers', async ({ request }) => {
    const { team, owner } = await seedTeam();
    const teamEmailAddr = `team-vis-env-${team.id}@test.signflow.com`;
    await seedTeamEmail({ email: teamEmailAddr, teamId: team.id });
