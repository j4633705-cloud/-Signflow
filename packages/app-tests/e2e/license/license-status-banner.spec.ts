...
import type { TCachedLicense } from '@signflow/lib/types/license';
import { seedUser } from '@signflow/prisma/seed/users';

import { apiSignin } from '../fixtures/authentication';

const LICENSE_FILE_NAME = '.signflow-license.json';
const LICENSE_BACKUP_FILE_NAME = '.signflow-license-backup.json';
...
const getLicenseFilePath = () => {
  // From packages/app-tests/e2e/license -> ../../../../apps/remix/.signflow-license.json
...
test.describe
  .skip('License Status Banner', () => {
    ...
    test('[ADMIN]: no banner when license file is missing', async ({ page }) => {
      ...
      await expect(page.getByText('This is an expired license instance of signflow')).not.toBeVisible();
    ...
    test.skip('[ADMIN]: global banner shows when EXPIRED with unauthorized usage', async ({ page }) => {
      ...
      await expect(page.getByText('This is an expired license instance of signflow')).toBeVisible();
      ...
      await expect(
        page.getByText('Your signflow instance is using features that are not part of your license.'),
      ).toBeVisible();
    });
