...
import { DocumentVisibility } from '@signflow/prisma/client';
import { seedTeamDocumentWithMeta } from '@signflow/prisma/seed/documents';
...
test('[ORGANISATIONS]: manage branding preferences', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Brand Website' }).fill('https://signflow.com');
  ...
  expect(teamSettings.brandingUrl).toEqual('https://signflow.com');
  ...
  expect(inheritedTeamSettings.brandingUrl).toEqual('https://signflow.com');
...
test('[ORGANISATIONS]: manage email preferences', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Reply to email' }).fill('organisation@signflow.com');
  ...
  // Check that the team settings have inherited these values.
  expect(teamSettings.emailReplyTo).toEqual('organisation@signflow.com');
  ...
  // Check that the team settings now inherit from organisation again.
  expect(inheritedTeamSettings.emailReplyTo).toEqual('organisation@signflow.com');
  ...
  expect(documentMeta.emailReplyTo).toEqual('organisation@signflow.com');
