...
import { createTeam } from '@signflow/lib/server-only/team/create-team';
import { nanoid } from '@signflow/lib/universal/id';
import { seedOrganisationMembers } from '@signflow/prisma/seed/organisations';
...
test('[ORGANISATIONS]: inherit members', async ({ page }) => {
  ...
  const memberEmail = `member-${nanoid()}@test.signflow.com`;
  const memberEmail2 = `member-2-${nanoid()}@test.signflow.com`;
  const memberEmail3 = `member-3-${nanoid()}@test.signflow.com`;
  const managerEmail = `manager-${nanoid()}@test.signflow.com`;
  const adminEmail = `admin-${nanoid()}@test.signflow.com`;
...
test('[ORGANISATIONS]: manage groups and members', async ({ page }) => {
  ...
  const memberEmail1 = `member-1-${nanoid()}@test.signflow.com`;
  const memberEmail2 = `member-2-${nanoid()}@test.signflow.com`;
  const memberEmail3 = `member-3-${nanoid()}@test.signflow.com`;
  const memberEmail4 = `member-4-${nanoid()}@test.signflow.com`;
  const memberEmail5 = `member-5-${nanoid()}@test.signflow.com`;
  const memberEmail6 = `member-6-${nanoid()}@test.signflow.com`;

  const adminEmail1 = `admin-1-${nanoid()}@test.signflow.com`;
