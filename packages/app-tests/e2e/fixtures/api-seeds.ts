...
} from '@signflow/trpc/server/envelope-router/distribute-envelope.types';
import type { TCreateEnvelopeRecipientsResponse } from '@signflow/trpc/server/envelope-router/envelope-recipients/create-envelope-recipients.types';
...
> => {
  ...
  const recipients = options.recipients ?? [
    {
      email: `signer-${Date.now()}@test.signflow.com`,
      name: 'Test Signer',
...
> => {
  const recipients = options.recipients ?? [
    options.directRecipient ?? {
      email: 'direct-template-recipient@signflow.com',
