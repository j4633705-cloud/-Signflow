...
import { seedBlankDocument, seedPendingDocumentWithFullFields } from '@signflow/prisma/seed/documents';
...
test('[DOCUMENT_FLOW]: should be able to approve a document', async ({ page }) => {
  ...
  const { recipients } = await seedPendingDocumentWithFullFields({
    owner: user,
    teamId: team.id,
    recipients: ['user@signflow.com', 'approver@signflow.com'],
...
test('[DOCUMENT_FLOW]: should be able to create, send with redirect url, sign a document and redirect to redirect url', async ({
  ...
  await page.getByLabel('Redirect URL').fill('https://signflow.com');
  ...
  await page.waitForURL('https://signflow.com');
