...
import { ZOrganisationNameSchema } from '@signflow/trpc/server/organisation-router/create-organisation.types';
import { z } from 'zod';
...
export const ZClaimFlagsSchema = z.object({
  /**
   * Allows disabling of signflow branding for:
   * - Certificates
   * - Emails
   * - Other?
   */
...
> = {
  ...
  hidePoweredBy: {
    key: 'hidePoweredBy',
    label: 'Hide signflow branding by',
  },
  emailDomains: {
    key: 'emailDomains',
    label: 'Email domains',
