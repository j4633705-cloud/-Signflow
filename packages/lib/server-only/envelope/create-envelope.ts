...
import type { PlaceholderInfo } from '@signflow/lib/server-only/pdf/auto-place-fields';
import { convertPlaceholdersToFieldInputs } from '@signflow/lib/server-only/pdf/auto-place-fields';
import { findRecipientByPlaceholder } from '@signflow/lib/server-only/pdf/helpers';
import { normalizePdf as makeNormalizedPdf } from '@signflow/lib/server-only/pdf/normalize-pdf';
...
import { nanoid, prefixedId } from '@signflow/lib/universal/id';
    ...
    if (itemsWithPlaceholders.length > 0) {
      ...
      if (shouldCreatePlaceholderRecipients) {
        ...
        const placeholderRecipients = Array.from(uniqueRecipientRefs.entries(), ([recipientIndex, name]) => ({
          envelopeId: envelope.id,
          email: `recipient.${recipientIndex}@signflow.com`,
          name,
