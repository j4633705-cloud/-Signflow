...
import type { BrandingSettings } from '@signflow/email/providers/branding';
...
import { EmailDomainStatus, type OrganisationClaim, type OrganisationGlobalSettings } from '@signflow/prisma/client';
...
export const getEmailContext = async (options: GetEmailContextOptions): Promise<EmailContextResponse> => {
  ...
  // system mailer + signflow sender, so the degraded organisation is findable.
  ...
  const resolvedTransportData = transportResolution
    ...
    : {
        name: SIGNFLOW_INTERNAL_EMAIL.name,
        address: SIGNFLOW_INTERNAL_EMAIL.address,
        transport: mailer,
      };
  ...
  const senderEmailId = match(meta?.emailId)
    ...
    .with(null, () => null) // Explicit null means to use the signflow email.
    .exhaustive();
