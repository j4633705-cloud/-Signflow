import { SIGNFLOW_ENCRYPTION_SECONDARY_KEY } from '@signflow/lib/constants/crypto';
import { symmetricDecrypt, symmetricEncrypt } from '@signflow/lib/universal/crypto';
import { z } from 'zod';
...
export const encryptEmailTransportConfig = (config: TEmailTransportConfig): string => {
  if (!SIGNFLOW_ENCRYPTION_SECONDARY_KEY) {
    throw new Error('Missing encryption key');
  }

  return symmetricEncrypt({
    key: SIGNFLOW_ENCRYPTION_SECONDARY_KEY,
...
export const decryptEmailTransportConfig = (encrypted: string): TEmailTransportConfig => {
  ...
  const decrypted = Buffer.from(symmetricDecrypt({ key: SIGNFLOW_ENCRYPTION_SECONDARY_KEY, data: encrypted })).toString(
    'utf-8',
  );
