...
import DocumentRejectedEmail from '@signflow/email/templates/document-rejected';
import DocumentRejectionConfirmedEmail from '@signflow/email/templates/document-rejection-confirmed';
import { isRecipientEmailValidForSending } from '@signflow/lib/utils/recipients';
import { prisma } from '@signflow/prisma';
import { createElement } from 'react';

import { getI18nInstance } from '../../../client-only/providers/i18n-server';
import { NEXT_PUBLIC_WEBAPP_URL } from '../../../constants/app';
import { SIGNFLOW_INTERNAL_EMAIL } from '../../../constants/email';
...
export const run = async ({ payload, io }: { payload: TSendSigningRejectionEmailsJobDefinition; io: JobRunIO }) => {
  ...
  // The owner notification below intentionally uses the internal signflow email, so it still sends.
  ...
  await io.runTask('send-owner-notification-email', async () => {
    ...
    await mailer.sendMail({
      ...
      from: SIGNFLOW_INTERNAL_EMAIL, // Purposefully using internal email here.
