...
const QUEUE_NAME = 'signflow-jobs';
...
export class BullMQJobProvider extends BaseJobProvider {
  ...
  private constructor() {
    ...
    const prefix = env('NEXT_PRIVATE_REDIS_PREFIX') || 'signflow';
  ...
  public override getApiHandler(): (c: HonoContext) => Promise<Response | void> {
    ...
    return async (c: HonoContext) => {
      ...
      if (env('NODE_ENV') !== 'development') {
        const { getOptionalSession } = await import('@signflow/auth/server/lib/utils/get-session');
