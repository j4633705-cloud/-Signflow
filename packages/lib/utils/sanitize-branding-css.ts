 ...
 * CSS nesting (`.signflow-branded { <user css> }`). The sanitiser itself
 ...
 */
export const SANITIZE_BRANDING_SCOPE_CLASS = 'signflow-branded';
 ...
 * The sanitised output is later wrapped in `.signflow-branded { ... }` via
...
const sanitizeRule = (rule: Rule, warnings: SanitizeBrandingCssWarning[]): void => {
  ...
  // the entire sanitised CSS in `.signflow-branded { ... }` (CSS nesting).
