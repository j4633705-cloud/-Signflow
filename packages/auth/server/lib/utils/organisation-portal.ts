...
import { formatOrganisationCallbackUrl } from '@signflow/lib/utils/organisation-authentication-portal';
...
export const getOrganisationAuthenticationPortalOptions = async (
  ...
  const clientSecret = Buffer.from(
    symmetricDecrypt({ key: SIGNFLOW_ENCRYPTION_KEY, data: encryptedClientSecret }),
  ).toString('utf-8');
