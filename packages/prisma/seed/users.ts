...
import { hashSync } from '@signflow/lib/server-only/auth/hash';
import { createPersonalOrganisation } from '@signflow/lib/server-only/organisation/create-organisation';
...
export const seedTestEmail = () => `${nanoid()}@test.signflow.com`;
...
export const seedUser = async ({
  ...
  if (!email) {
    email = `${nanoid()}@test.signflow.com`;
  }
