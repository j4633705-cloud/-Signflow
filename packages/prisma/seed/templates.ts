...
} from '@signflow/lib/constants/direct-templates';
import { incrementTemplateId } from '@signflow/lib/server-only/envelope/increment-id';
...
export const seedTemplate = async (options: SeedTemplateOptions) => {
  ...
  return await prisma.envelope.create({
    data: {
      ...
      recipients: {
        create: {
          email: 'recipient.1@signflow.com',
          name: 'Recipient 1',
