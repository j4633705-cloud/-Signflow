...
This document defines the writing conventions for signflow documentation.
...
1. **Users** - People using the signflow web application to send and sign documents
2. **Developers** - Building integrations with the API or SDKs
3. **Self-hosters** - Running their own signflow instance
...
```typescript
// Basic usage
const document = await signflow.documents.create({
  title: "Contract",
  file: pdfBuffer,
});
