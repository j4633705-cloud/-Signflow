...
if [ ! -z "$DOCKER_REPOSITORY" ]; then
    ...
    docker tag "signflow-base" "$DOCKER_REPOSITORY:latest"
    docker tag "signflow-base" "$DOCKER_REPOSITORY:$GIT_SHA"

    # Add version tag if available
    if [ ! -z "$APP_VERSION" ] && [ "$APP_VERSION" != "undefined" ]; then
        docker tag "signflow-base" "$DOCKER_REPOSITORY:$APP_VERSION"
    fi
else
    echo "Using default repositories: dockerhub and ghcr.io"
    
    # Add tags for both default repositories
    docker tag "signflow-base" "signflow/signflow:latest"
    docker tag "signflow-base" "signflow/signflow:$GIT_SHA"
    docker tag "signflow-base" "ghcr.io/signflow/signflow:latest"
    docker tag "signflow-base" "ghcr.io/signflow/signflow:$GIT_SHA"

    # Add version tags if available
    if [ ! -z "$APP_VERSION" ] && [ "$APP_VERSION" != "undefined" ]; then
        docker tag "signflow-base" "signflow/signflow:$APP_VERSION"
        docker tag "signflow-base" "ghcr.io/signflow/signflow:$APP_VERSION"
    fi
fi

# Remove the temporary base tag
docker rmi "signflow-base"
